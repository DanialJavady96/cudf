  ...
  cudf::size_type const max_size)
{
  ...
  auto left_index = cudf::detail::grid_1d::global_thread_id();

  unsigned int const activemask = __ballot_sync(0xffff'ffffu, left_index < left_num_rows);
  ...
  if (outer_row_index < outer_num_rows) {
    bool found_match = false;
    for (int64_t cur_right(0); cur_right < inner_num_rows;
         ++cur_right) {
      auto output_dest           = cudf::ast::detail::value_expression_result<bool, has_nulls>();

      evaluator.evaluate(
        output_dest, left_index, cur_right, 0, thread_intermediate_storage);

      if (output_dest.is_valid() && output_dest.value()) { 
        if(join_type == join_kind::LEFT_SEMI_JOIN && !found_match) {
          add_left_to_cache(left_index,
                        current_idx_shared,
                        warp_id,
                        join_shared_l[warp_id]);
        }  
